...
import { PencilIcon, FileText, BookOpen, UserIcon, MapPin, Tag, Users } from "lucide-react";
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <div className="mb-2 flex items-center gap-2">
                      <div className="h-10 w-10 rounded-full bg-green-500 flex items-center justify-center text-white font-bold">ID</div>
                      <div>
          ...
          : <li>Nenhum e-mail registrado.</li>}
            ...
            <Card className="border-green-200 bg-green-50 mt-6">
              <CardContent className="p-6 flex items-center justify-between">
                ...
                <Link to={`/trabalhos?orcid=${orcidId}`}>
                  <Button className="bg-green-600 hover:bg-green-700">Ver trabalhos</Button>
                </Link>
              </CardContent>
            </Card>
              ...
              <Card><CardContent className="p-6">
                ...
                <div className="flex justify-between">
                  <div>
                    ...
                    <p className="font-medium">{nome} {sobrenome}</p>
                  </div>
                  <Badge variant="outline" className="bg-green-50 text-green-600 border-green-200">Visível</Badge>
                </div>
              </CardContent></Card>
            </section>
              ...
              <Card><CardContent className="p-6">
                ...
                <ul className="list-disc pl-5">
                  {outrosNomes.length > 0
                    ? outrosNomes.map((nome, idx) => <li key={idx}>{nome.content}</li>)
                ...
                <div className="flex flex-wrap gap-2">
                  {palavrasChave.length > 0
                    ? palavrasChave.map((palavra, idx) => (
                      <Badge key={idx} variant="outline" className="bg-green-50 text-green-600 border-green-200">{palavra}</Badge>
                    ))
            ...
            <section className="scroll-mt-16 mt-6">
              <Card><CardContent className="p-6">
                ...
                <ul className="list-disc pl-5">
                  {enderecos.length > 0
                    ? enderecos.map((end, idx) => <li key={idx}>{end?.country?.value}</li>)
          ...
          : <li>Nenhum emprego registrado.</li>}
            ...
            <section className="scroll-mt-16 mt-6">
              <Card><CardContent className="p-6">
                ...
                <ul className="list-disc pl-5">
                  {afiliacoes.length > 0
                    ? afiliacoes.map((afil, idx) => <li key={idx}>{afil?.organization?.name}</li>)
